...
function adminControlState(row){
  if(!row.labora||row.cierre_estado==='no_aplica')return {key:'no_aplica',label:'No programado',tone:'neutral'};
...
function renderAdminControl(){
  ...
  for(const row of rows){
    ...
    html+=`<article class="admin-control-row" data-tone="${state.tone}">
      ...
      <span data-label="Entrada"><b>${esc(adminControlTime(row.entrada_at))}</b><small>${esc(row.entrada_estado||'Sin marca')}</small></span>
      <span data-label="Evidencias" class="${issue?'has-issue':pending?'needs-attention':'is-ok'}" ${issue?`title="${esc(issue.detalle)}"`:''}><b>${esc(evidenceLabel)}</b><small>${issue?esc(issue.detalle):pending?'Bloquea la salida':'Requisitos del día'}</small></span>
      ...
      <span data-label="Salida"><b>${esc(adminControlTime(row.salida_at))}</b><small>${row.salida_at?'Registrada':'Sin registro'}</small></span>
...
async function loadAdminControl(){
  ...
  const [{data,error},{data:issueData,error:issueError}]=await Promise.all([db.rpc('dash_admin_control_diario',{p_fecha:date}),db.rpc('dash_supervision_impedimentos',{p_fecha:date})]);
  ...
  data.filas=(data.filas||[]).map(row=>({...row,impedimentos:byIssue.get(String(row.colaborador_id))||[]}));APP.adminControl=data;fillAdminControlAreas();renderAdminControl();
}
