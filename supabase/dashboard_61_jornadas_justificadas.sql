-- DASHBOARD 61: jornadas justificadas en Cierres y entregables.
-- Ejecutar despues de dashboard_60_revision_sin_canceladas.sql.
-- Conserva marcas y evidencias originales. J solo requiere Facebook en esta vista.
begin;
do $$
begin
  if to_regprocedure('public.dash_admin_cierres_base_61(date)') is null then
    alter function public.dash_admin_cierres(date) rename to dash_admin_cierres_base_61;
  end if;
end;
$$;
revoke all on function public.dash_admin_cierres_base_61(date) from public,anon,authenticated;
create or replace function public.dash_admin_cierres(p_fecha date)
returns jsonb language plpgsql stable security definer set search_path=public
as $$
declare v_data jsonb; v_personas jsonb;
begin
  v_data:=public.dash_admin_cierres_base_61(p_fecha);
  if not coalesce((v_data->>'ok')::boolean,false) then return v_data; end if;
  select coalesce(jsonb_agg(
    jsonb_set(fila.item,'{cierre}',coalesce(fila.item->'cierre','{}'::jsonb)||
      jsonb_build_object('justificado',coalesce(registro.estado='J',false))
    ) order by fila.orden),'[]'::jsonb)
  into v_personas
  ...
  left join public.asis_registros registro
    on registro.colaborador_id=(fila.item->>'id')::bigint and registro.fecha=p_fecha;
  return jsonb_set(v_data,'{personas}',v_personas);
end;
$$;
...
begin
  ...
  if p_requisito<>'comparticiones' and exists(
    select 1 from public.asis_registros
    where colaborador_id=p_colaborador and fecha=p_fecha and estado='J'
  ) then return jsonb_build_object('ok',false,'motivo','jornada_justificada'); end if;
...
begin
  ...
  if coalesce((v_resumen->>'aplica_jornada')::boolean,false)
    ...
    select * into v_reg from public.asis_registros
     where colaborador_id=p_colaborador and fecha=p_fecha for update;
    if v_reg.id is not null and v_reg.estado<>'J' and v_reg.salida_at is null then
