-- Aplicar despues de dashboard_67. Excepcion para Alviery, incluido su historial.
-- Conserva horarios, marcaciones, horas y evidencias originales.
begin;
do $$ begin
  if (select count(*) from public.asis_colaboradores where dni='73939131')<>1 then
    raise exception 'Se requiere una coincidencia unica del DNI de Alviery';
  end if;
  if to_regprocedure('public.dash_cierre_resumen_colab_base_68(bigint,date)') is null then
    alter function public.dash_cierre_resumen_colab(bigint,date) rename to dash_cierre_resumen_colab_base_68;
  end if;
end $$;
revoke all on function public.dash_cierre_resumen_colab_base_68(bigint,date) from public,anon,authenticated;

create or replace function public.dash_cierre_resumen_colab(p_colaborador bigint,p_fecha date)
returns jsonb language plpgsql stable security definer set search_path=public as $$
declare
  v_data jsonb; v_req jsonb; v_pending integer; v_state text;
begin
  v_data:=public.dash_cierre_resumen_colab_base_68(p_colaborador,p_fecha);
  if not coalesce((v_data->>'ok')::boolean,false)
    or not exists(select 1 from public.asis_colaboradores where id=p_colaborador and dni='73939131')
  ...
  select coalesce(jsonb_agg(item order by orden),'[]'::jsonb),
    ...
    from jsonb_array_elements(coalesce(v_data->'requisitos','[]'::jsonb)) with ordinality filas(item,orden)
    where item->>'tipo'='comparticiones';
  v_state:=case
    when not coalesce((v_data->>'aplica')::boolean,false) then v_data->>'estado'
    when coalesce((v_data->>'justificado')::boolean,false) then 'justificado'
    when coalesce((v_data->>'aplica_jornada')::boolean,false) and v_data->>'entrada_at' is null then 'sin_entrada'
    when v_pending=0 then 'completa'
    when coalesce((v_data->>'comparticiones_vencidas')::boolean,false) then 'incompleta'
    else 'en_curso' end;
  return v_data||jsonb_build_object(
    'solo_asistencia_comparticiones',true,'requiere_salida',false,
    'requisitos',v_req,'asignaciones','[]'::jsonb,'estado',v_state,
    'pendientes',v_pending,'pendientes_jornada',0,'pendientes_salida',0,
    'puede_marcar_salida',false,'comparticiones_pendientes',v_pending>0);
end $$;
revoke all on function public.dash_cierre_resumen_colab(bigint,date) from public,anon,authenticated;

-- El ranking mantiene la asistencia y Facebook, sin penalizar salida ni asignaciones.
...
begin
  ...
  with calendario as materialized (
    ...
    select d.*,ev.*,
      ...
      incorporado and trabajo and not coalesce((cierre->>'solo_asistencia_comparticiones')::boolean,false) and fecha>=date '2026-09-07' and estado in ('P','T') and salida_at between
  ...
  return jsonb_build_object('version',3,'salidas_desde',date '2026-09-07','mes',v_inicio,'hasta',v_fin,'filas',v_filas,
